    ...
    <Shell>
      <div className="fade-in space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="disp text-2xl font-bold flex items-center gap-2"><Library size={22} className="text-[--gold-deep]" /> Msingi Library</h1>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button onClick={() => setBookmarkedOnly((b) => !b)}
              ...
              style={{ borderColor: "var(--gold-deep)", background: bookmarkedOnly ? "var(--gold-deep)" : "white" }}>
              ...
              <div key={r.id} className="brick bg-white rounded-2xl p-5 border flex flex-col" style={{ borderColor: "var(--slate)" }}>
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <button onClick={() => toggleBookmark(r)} title="Bookmark" className="tap">
                    <Bookmark size={16} fill={r.bookmarked ? "var(--gold-deep)" : "none"} color="var(--gold-deep)" />
                  </button>
                </div>
                ...
                <div className="mt-auto pt-2">
                  {r.bodyText ? (
                    <button onClick={() => setReading(r)} className="tap text-xs font-semibold text-[--gold-deep]">Read →</button>
                  ) : r.fileUrl ? (
                    <a href={r.fileUrl} target="_blank" rel="noopener noreferrer" className="tap text-xs font-semibold text-[--gold-deep]">Open file →</a>
                  ) : (
