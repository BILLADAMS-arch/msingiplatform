  ...
  return (
    <Shell name={data?.profile?.name} xp={data?.profile?.xp} streak={data?.profile?.streak}>
      <div className="fade-in space-y-8">
        <div>
          <h1 className="disp text-3xl font-bold">Good morning, {data?.profile?.name || "learner"} 👋</h1>
        ...
        {!data ? (
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              <StatCard icon={<Flame size={18} />} label="Learning Streak" value={`${data.profile?.streak ?? 1}d`} tone="gold" />
              <StatCard icon={<Star size={18} />} label="XP" value={data.profile?.xp ?? 0} tone="gold" />
              <StatCard icon={<Layers size={18} />} label="Overall Progress" value={`${overallProgress}%`} tone="blue" />
              <StatCard icon={<Target size={18} />} label="Avg Score" value={avgScore !== null ? `${avgScore}%` : "—"} tone="blue" />
            ...
            <div className="brick bg-white rounded-2xl p-5 border" style={{ borderColor: "var(--slate)" }}>
              <div className="flex items-center justify-between mb-3">
                ...
                <Link href="/learn" className="text-xs font-semibold text-[--primary]">See all</Link>
              </div>
              {!continueLoaded ? (
                ...
                <div className="flex items-center justify-between bg-[--stone-2] rounded-xl p-4">
                  <div>
                    <div className="text-xs font-semibold mb-0.5 flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 rounded-full" style={{ background: subjectAccent(continueLearning.subjectName).color }} />
                      <span style={{ color: subjectAccent(continueLearning.subjectName).color }}>{continueLearning.subjectName}</span>
                    </div>
                    <div className="font-semibold">{continueLearning.topicName}</div>
                    <div className="w-40 mt-2"><FoundationBar pct={continueLearning.masteryPct} tone="green" /></div>
                    <div className="text-xs text-[--ink-soft] mt-1">{continueLearning.masteryPct}% mastery</div>
                  </div>
                  <Link href={`/learn/lesson/${continueLearning.lessonId}`} className="tap px-4 py-2 rounded-full text-sm font-semibold text-white" style={{ background: "var(--primary)" }}>Continue</Link>
                </div>
              ) : (
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="brick bg-white rounded-2xl p-5 border" style={{ borderColor: "var(--slate)" }}>
                <h3 className="disp font-bold mb-3 flex items-center gap-2"><Sparkles size={16} className="text-[--primary]" /> Recommended For You</h3>
                {weakTopics.length > 0 ? (
                  <div className="space-y-2">
                    {weakTopics.map(([t, v]) => (
                      <div key={t} className="flex items-center justify-between text-sm">
                        <span>Revise <b>{t}</b> — {v}% mastery</span>
                        <Link href={`/practice?topic=${encodeURIComponent(t)}`} className="text-xs font-semibold text-[--primary]">Revise</Link>
                      </div>
                    ))}
                  </div>
                ) : (
